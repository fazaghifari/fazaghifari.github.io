...
Supposed that we have any matrix $Y$, we can decompose $Y$ into $U$, $\Sigma$, and $V$ matrices (I will not dive into the details of the matrix decomposition, as nowadays we can use library such as numpy to perform that. However, if you are interested I strongly recommend [this video](https://www.youtube.com/watch?v=mBcLRGuAFUk) by Prof. Gilbert Strang himself 😉) 

$$
Y = U\Sigma V^T
$$

In the context of our fluid flow POD, the $Y$ matrix is our snapshot matrix. Matrix $U$ can be interpreted as a matrix that "represents" spatial components. Matrix $V$ as a matrix that "represents" the other parameters components, or in this case, the time component. $\Sigma$ is a diagonal matrix that represents the "strength" or the importance of the corresponding basis.
